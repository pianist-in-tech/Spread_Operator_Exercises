...
const findMin = (...args) => args.reduce(smallest, num) => {
 if(smallest<=num) return smallest;
}
...
const doubleAndReturnArgs = (arr, ...arguments)=> {
    let doubledArgs = arguments.map(num) => (num*2);
...
const removeKey = (obj, key) => ({[key]:_, ...obj} = obj);
